  ...
  if (active && payload && payload.length > 0 && startPrice !== undefined) {
    ...
    return (
      <div className="bg-white p-4 border border-gray-200 rounded-lg shadow-lg min-w-[150px] z-50">
        ...
        <p className="text-xl font-mono font-bold text-gray-900 mb-1">${currentPrice}</p>
...
export default function StockPage({ stock }) {
  ...
  // --- RESTORED LOGIC WITH LOWER LIMIT (100) ---
  const fullDesc = stock.description ? String(stock.description) : "No description available.";
  const CHAR_LIMIT = 100; // Force button to appear for almost everyone
  
  const shortDesc = fullDesc.length > CHAR_LIMIT 
    ? fullDesc.slice(0, CHAR_LIMIT) + "..." 
    : fullDesc;
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 mb-6 flex justify-between items-center transition-all">
            <div>
                ...
                <div className="flex items-center gap-4 mt-2">
                    <span className="text-4xl font-mono tracking-tighter font-bold">${displayPrice}</span>
            ...
            <div className="text-right hidden md:block">
                ...
                <p className="font-bold text-xl">{formatLargeNumber(stock.market_cap)}</p>
            </div>
        </div>

        {/* --- COMPANY PROFILE (With Read More Restored) --- */}
        <div className="mb-8">
            <div className="flex gap-2 mb-3">
                <span className="bg-blue-100 text-blue-800 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide">{stock.sector}</span>
                <span className="bg-gray-100 text-gray-600 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide">{stock.industry}</span>
            </div>
                ...
                <p>
                    ...
                    {shouldShowButton && (
                        <button 
                            ...
                            className="text-blue-600 font-bold ml-2 hover:underline focus:outline-none text-xs uppercase tracking-wide"
                        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="flex justify-between items-end mb-4">
                        ...
                        <div className="flex gap-1 bg-gray-100 p-1 rounded-lg h-fit">
                            {['1M', '6M', 'YTD', '1Y', 'ALL'].map((range) => (
                                <button key={range} onClick={() => setTimeRange(range)} className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${timeRange === range ? "bg-white shadow text-black" : "text-gray-500 hover:text-gray-900"}`}>{range}</button>
                            ))}
                        </div>
                    </div>
                    <div className="h-72 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={chartData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                                ...
                                <XAxis dataKey="date" hide padding={{ left: 20, right: 20 }} />
                                <YAxis domain={['auto', 'auto']} orientation="right" tickFormatter={(number) => `$${number}`} tick={{fontSize: 12, fill: '#9ca3af'}} axisLine={false} tickLine={false} width={60} />
                                <Tooltip cursor={{ stroke: '#6b7280', strokeWidth: 1, strokeDasharray: '4 4' }} content={<CustomTooltip startPrice={chartStartPrice} />} />
                    ...
                    <div className="flex justify-between items-end mb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="relative group flex items-center justify-center pt-0.5">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-gray-400 hover:text-blue-700 cursor-help transition-colors">
                                  <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm8.706-1.442c1.146-.573 2.437.463 2.126 1.706l-.709 2.836.042-.02a.75.75 0 01.67 1.34l-.04.022c-1.147.573-2.438-.463-2.127-1.706l.71-2.836-.042.02a.75.75 0 11-.671-1.34l.041-.022zM12 9a.75.75 0 100-1.5.75.75 0 000 1.5z" clipRule="evenodd" />
                                </svg>
                                <div className="absolute left-6 top-0 w-64 p-3 bg-gray-900 text-white text-xs rounded shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 pointer-events-none border border-gray-700">
                                    <p className="font-bold mb-1 border-b border-gray-700 pb-1 text-gray-200">RISK METRIC GUIDE</p>
                                    <p className="leading-relaxed mt-1 text-gray-300">This chart visualizes "Pain from Peak."<br/><br/>It tracks the percentage loss an investor would currently face if they had bought at the highest possible price in the specific period.</p>
                                </div>
                            </div>
                        </div>
                        <div className="flex gap-1 bg-gray-100 p-1 rounded-lg h-fit">
                    ...
                    <div className="h-48 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={chartData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                                ...
                                <XAxis dataKey="date" hide padding={{ left: 20, right: 20 }} />
                                <YAxis orientation="right" tickFormatter={(val) => `${val}%`} tick={{fontSize: 11, fill: '#9ca3af'}} axisLine={false} tickLine={false} width={40} />
                                <Tooltip cursor={{ stroke: '#ef4444', strokeWidth: 1 }} contentStyle={{backgroundColor: '#fff', borderRadius: '8px', border: '1px solid #e5e7eb'}} itemStyle={{color: '#ef4444', fontWeight: 'bold'}} labelStyle={{color: '#6b7280', fontSize: '12px'}} formatter={(value) => [`${value}%`, "Drawdown"]} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h3 className="text-sm uppercase text-gray-500 font-bold mb-6">Corporate Profile</h3>
                    <div className="space-y-4">
                        <div className="flex justify-between border-b border-gray-100 pb-3">
                            <span className="text-gray-500 text-xs font-medium uppercase">CEO</span>
                            <span className="font-bold text-gray-900 text-sm text-right">{stock.ceo || "N/A"}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-100 pb-3">
                            <span className="text-gray-500 text-xs font-medium uppercase">Employees</span>
                            <span className="font-bold text-gray-900 text-sm text-right">{stock.employees ? stock.employees.toLocaleString() : "N/A"}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-100 pb-3">
                            <span className="text-gray-500 text-xs font-medium uppercase">Headquarters</span>
                            <span className="font-bold text-gray-900 text-sm text-right max-w-[60%]">{stock.hq || "N/A"}</span>
                        </div>
                        <div className="flex justify-between pt-1">
                            <span className="text-gray-500 text-xs font-medium uppercase">Website</span>
                            <a href={stock.website} target="_blank" rel="noopener noreferrer" className="font-bold text-blue-600 text-sm hover:underline truncate max-w-[150px] text-right">
                ...
                {/* 3. VITAL SIGNS */}
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h3 className="text-sm uppercase text-gray-500 font-bold mb-6">Investor Vital Signs</h3>
                    <div className="grid grid-cols-2 gap-y-6 gap-x-4">
                        <div>
                            <p className="text-[10px] uppercase text-gray-400 font-bold">Profit Margin</p>
                            <p className={`text-xl font-mono font-bold ${stock.profit_margin > 15 ? 'text-green-600' : 'text-gray-900'}`}>{stock.profit_margin}%</p>
                        ...
                        <div>
                            ...
                            <p className={`text-xl font-mono font-bold ${stock.roe > 15 ? 'text-green-600' : 'text-gray-900'}`}>{stock.roe}%</p>
                        ...
                        <div>
                            ...
                            <p className="text-xl font-mono font-bold text-blue-600">${stock.target_price ? Number(stock.target_price).toFixed(2) : "N/A"}</p>
                        ...
                        <div>
                            ...
                            <p className="text-xl font-mono font-bold text-gray-900">${stock.high_52}</p>
                ...
                {stock.financials && (
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <h3 className="text-sm uppercase text-gray-500 font-bold mb-6">Financial Flow ({stock.financials.year})</h3>
                        ...
                        <p className="text-xs text-gray-400 mt-6 text-center">*Simplified flow based on {stock.financials.year} report.</p>
                    </div>
                )}
            </div>
        </div>
